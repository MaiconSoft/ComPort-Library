(* *****************************************************
  ...
  *      open                                          *
  *    Add EnumComPortsLinux, for enumerate ports when *
  *     delphi programs run on wine (linux)            *
  *                                                    *
  **************************************************** *)
...
{$ENDIF}
    ...
    function WriteAnsiStrAsync(var Str: AnsiString;
      var AsyncPtr: PAsync): Integer;
...
begin
  ...
  begin
    ...
    Reg.WriteString('FlowControl.XonXoffOut',
      BoolToStr(FlowControl.XonXoffOut));
    Reg.WriteString('FlowControl.XoffChar',
      AnsiCharToStr(FlowControl.XoffChar));
...
begin
  ...
  begin
    ...
    FlowControl.XonXoffOut :=
      ...
      BoolToStr(FlowControl.XonXoffOut)));
    FlowControl.XoffChar := StrToAnsiChar(IniFile.ReadString(Name,
      'FlowControl.XoffChar', AnsiCharToStr(FlowControl.XoffChar)));
    FlowControl.XonChar := StrToAnsiChar(IniFile.ReadString(Name,
      'FlowControl.XonChar', AnsiCharToStr(FlowControl.XonChar)));
  end;
end;
...
begin
  ...
  begin
    ...
    FlowControl.XoffChar :=
      StrToAnsiChar(Reg.ReadString('FlowControl.XoffChar'));
...
// Add portability on linux, using wine
function EnumComPortsLinux(Ports: TStrings): Boolean;
var
  Index: Integer;
  Data: string;
  sr: TSearchRec;
...
begin
  if EnumComPortsLinux(Ports) then // If ports on linux detected
    Exit;                          // then ignone windows mode
